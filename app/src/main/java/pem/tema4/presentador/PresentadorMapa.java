...
public class PresentadorMapa implements IPresentadorMapa {
    ...
    private BroadcastReceiver receptorAvisos = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(AppMediador.AVISO_ESTADO_INICIAL)){
                ArrayList<DatoMarca> datosMarca = (ArrayList<DatoMarca>) intent.getSerializableExtra(AppMediador.CLAVE_MARCAS);

                for(int i = 0; i<datosMarca.size(); i++){
                    Object[] datos = new Object[3];
                    datos[0] = datosMarca.get(i).getLatitud();
                    datos[1] = datosMarca.get(i).getLongitud();
                    datos[2] = datosMarca.get(i).getTitulo();
                ...
                if ( intent.getExtras() != null ) {
                    ...
                    String titulo = intent.getStringExtra(AppMediador.CLAVE_TITULO);
                    marcaSeleccionada.setPosition(new LatLng(latitud, longitud));
                    marcaSeleccionada.setTitle(titulo);*/
    ...
    public void actualizarMapa(int estado, Object datos) {
        ...
        switch (estado){
            ...
            case AppMediador.ESTADO_BORRAR_MARCA:
                ...
                Object[] datosMarca = new Object[3];
                datosMarca[0] = marcaSeleccionada.getPosition().latitude;
                datosMarca[1] = marcaSeleccionada.getPosition().longitude;
                datosMarca[2] = marcaSeleccionada.getTitle();
                modelo.borrarMarca(datosMarca);
                break;
        }
    }
    ...
    public void tratarToqueMapa(double latitud, double longitud) {
        ...
        vistaMapa.mostrarDialogo(marca);
    }
    ...
    public void editarMarca(Object datos) {
        marcaSeleccionada = (Marker) datos;
        String[] datosMarca = new String[3];
        datosMarca[0] = Double.toString(marcaSeleccionada.getPosition().latitude);
        datosMarca[1] = Double.toString(marcaSeleccionada.getPosition().longitude);
        datosMarca[2] = marcaSeleccionada.getTitle();
        vistaMapa.mostrarInformacion(datosMarca);
    }
